...
const CompanyData: React.FC = ({ history, match }: any) => {
    ...
    useEffect(() => {
        if(!state[Scopes.PSP]?.credentials.length) return;
        const relevantCredential = state[Scopes.PSP].credentials.filter((c: any) => c.credential?.type.includes(CitizenCredentialConfig.template.type.pop()))?.[0]?.credential;
