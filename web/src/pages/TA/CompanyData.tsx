...
const CompanyData: React.FC = ({ history, match }: any) => {
    ...
    function onSubmit() {
        dispatch?.({type: Actions.SET_ISSUANCE_DATA, issuanceData: values, scope: Scopes.TA});
        navigate(nextStep);
    }

    useEffect(() => {
        if (!state[Scopes.TA]?.credentials.length) return;
        setRelevantCredential(state[Scopes.TA].credentials.filter((c: any) => c.credential?.type.includes(VehicleInformationCredentialConfig.template.type.pop()))?.[0]?.credential);
        ...
        <Layout>
            <div className='company-data-page-wrapper'>
                ...
                <section>
                    ...
                    <p>to <b>{state.TA?.connectedDID}</b></p> {/* TODO */} 
