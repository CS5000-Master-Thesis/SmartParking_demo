...
const ReceiveCredentials: React.FC = () => {
    ...
    useEffect(() => {
        dispatch?.({
            ...
            scope: Scopes.TA,
            credentials: [{type: VehicleRegistrationCredentialConfig.template.type.at(-1) as string, data: state.TA?.issuanceData}],
            issuer: Issuers.TA //TODO: should be COMPANY_HOUSE?
        })
    }, [dispatch]);

    useEffect(() => {
        if(state[Scopes.TA]?.issuanceComplete) {
            goToNextStep();
        }
    }, [state, goToNextStep])
            ...
            <RandomGraphicElement elements={5}>
                <div className='scan-qr-page-wrapper'>
                    ...
                    <div className='qr-wrapper'>
                        ...
                        <QRCode text={state[Issuers.TA]?.QRcontent ?? ""} />
                    </div>
